...
export default function Accueil({ onNavigate }: AccueilProps) {
  ...
  return (
    <div className="min-h-screen">
      <section className="relative h-[600px] flex items-center justify-center text-white" style={{ backgroundColor: '#0F2A44' }}>
        <div className="absolute inset-0 bg-[#1F4E79] opacity-20"></div>
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              ...
              className="bg-[#C9A24D] text-[#0F2A44] px-8 py-4 rounded-lg font-semibold text-lg hover:bg-[#E0B973] transition-colors"
            >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {services.map((service, index) => (
              <div
                ...
                onClick={() => onNavigate('services')}
              >
                <div className="text-[#0F2A44] group-hover:text-[#C9A24D] transition-colors mb-4">{service.icon}</div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid md:grid-cols-3 gap-12 text-center">
            <div>
              ...
              <p className="text-xl text-[#0F2A44]">Années d'expertise</p>
            </div>
            <div>
              <div className="text-5xl font-bold text-[#C9A24D] mb-3">200+</div>
              <p className="text-xl text-[#0F2A44]">Clients accompagnés</p>
            </div>
            <div>
              <div className="text-5xl font-bold text-[#C9A24D] mb-3">24/7</div>
              <p className="text-xl text-[#0F2A44]">Veille médiatique</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <button
            onClick={() => onNavigate('contact')}
            className="bg-[#0F2A44] text-white px-10 py-4 rounded-lg font-semibold text-lg hover:bg-[#1F4E79] transition-colors"
          >
            Prendre contact
          </button>
        </div>
